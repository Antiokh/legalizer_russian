...
def _looks_like_subjectless_prefix(prefix: str) -> bool:
    ...
    if caps:
        ...
        visible_caps = [token for token in caps if token.casefold() != first_word or first_word not in ignored_sentence_starters]
        if visible_caps:
            return False
...
def implicit_obligation_subject_occurrences(text: str) -> list[ImplicitObligationSubject]:
    ...
    aliases = sorted({party.alias for party in extract_party_aliases(text)}, key=str.casefold)
    if not aliases:
        return []
